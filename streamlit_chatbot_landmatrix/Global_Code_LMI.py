...
input_file = os.path.join(current_dir, 'rag__DATA.csv')
...
def REST(URL):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return data  # For example, return the data
...
def ChatBot_response( llm, parser,question):
    ...
    # combined_data = REST(url)
        
    return url


# Create a dictionary mapping country_id to country.name
#--------------------------------------------------------

def Mapping_groups(response, country_dict):
    data_for_df = []

    # Create a dictionary to map each value to its group
    value_to_group = {
        "BIOFUELS": "Agriculture",
        "BIOMASS_ENERGY_GENERATION": "Agriculture",
        "FODDER": "Agriculture",
        "FOOD_CROPS": "Agriculture",
        "LIVESTOCK": "Agriculture",
        "NON_FOOD_AGRICULTURE": "Agriculture",
        "AGRICULTURE_UNSPECIFIED": "Agriculture",
        "BIOMASS_ENERGY_PRODUCTION": "Forestry",
        "CARBON": "Forestry",
        "FOREST_LOGGING": "Forestry",
        "TIMBER_PLANTATION": "Forestry",
        "FORESTRY_UNSPECIFIED": "Forestry",
        "SOLAR_PARK": "Renewable energy power plants",
        "WIND_FARM": "Renewable energy power plants",
        "RENEWABLE_ENERGY": "Renewable energy power plants",
    ...
    # Iterate over each JSON object in the response list
    for item in response:
        # Extract necessary fields and add them to the list
        country_name = country_dict.get(item['country_id'], "Unknown")
        selected_version = item.get('selected_version', {})
        current_intention_of_investment = selected_version.get('current_intention_of_investment', [])
        
        # Map each investment intention to its group
        groups = [value_to_group.get(value, "Unknown") for value in current_intention_of_investment]
        group_str = ', '.join(set(groups))  # Convert to a unique string
        
        data_for_df.append({
            'id': item['id'],
            'country_id': item['country_id'],
            'country_name': country_name,
            'fully_updated_at': item['fully_updated_at'],
            'region_id': item['region_id'],
            'deal_size': selected_version.get('deal_size', 0),
            'current_contract_size': selected_version.get('current_contract_size', 0),
            'current_negotiation_status': selected_version.get('current_negotiation_status', "Unknown"),
            'current_implementation_status': selected_version.get('current_implementation_status', "Unknown"),
            'current_intention_of_investment': ', '.join(current_intention_of_investment),  # Concatenate intentions
            'group': group_str  # Add the corresponding group
        })
